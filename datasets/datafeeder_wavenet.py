...
class DataFeederWavenet(threading.Thread):
    ...
    def make_batches(self):
        ...
        for data_dir in self.data_dirs:
            example = [
                self._get_next_example(data_dir)
                for _ in range(int(n * 32 // len(self.data_dirs)))
            ]
    ...
    def _get_next_example(self, data_dir):
        ...
        while True:
            ...
            data_path = os.path.join(
                data_dir, data_paths[self._offset[data_dir]]
            )  # npz파일 1개 선택
            ...
            if os.path.exists(data_path):
                data = np.load(
                    data_path
                )  # data속에는 'audio', 'mel', 'linear', 'time_steps', 'mel_frames', 'text', 'token'
            else:
                continue
        ...
        if self.test_mode == False:  # test_mode에서는 전체. train_mode에서는 sample_size 만큼만
            s = np.random.randint(
                0, len(local_condition) - self.max_frames + 1
            )  # hccho
            ts = s * self.hop_size
...
if __name__ == "__main__":
    ...
    data_dirs = [
        "/Users/jhyunwoo/Documents/code/Tacotron2-Wavenet-Korean-TTS/datasets/psc"
    ]
    mydatafeed = DataFeederWavenet(
        coord,
        data_dirs,
        batch_size=5,
        receptive_field=1200,
        gc_enable=True,
        queue_size=8,
    )
    ...
    with tf.Session() as sess:
        try:
            ...
            while not coord.should_stop():
                a, b, c = sess.run(
                    [
                        mydatafeed.inputs_wav,
                        mydatafeed.local_condition,
                        mydatafeed.speaker_id,
                    ]
                )
